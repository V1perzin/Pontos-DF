...
export default function RegisterForm({ onRegister, apiUrl }){
  const [name,setName]=useState(''); const [email,setEmail]=useState(''); const [password,setPassword]=useState('');
  const [err,setErr]=useState(null);
  const submit = async () => {
    setErr(null);
    try{
      const res = await fetch((apiUrl||'http://localhost:4000') + '/auth/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,email,password})});
  ...
  return (
    <div style={{padding:20}}>
      ...
      <div style={{marginTop:10}}><button className="btn" onClick={submit}>Registrar</button></div>
    </div>
  )
}